...
function sysCall_init()
    ...
    ct_robot_joint1 = sim.getObjectHandle("ct_robot_prismatic_joint1")
    collisionHandle = sim.getCollisionHandle("Collision")
...
function sysCall_actuation()
    ...
    i = i + 1
    local position = sim.getJointPosition(ct_robot_joint1)
    --sim.setJointPosition(ct_robot_joint1, 0)--math.sin(i/5)/3)
...
function sysCall_sensing()
    ...
    steps_per_second = i / diff

    return_val = sim.readCollision(collisionHandle)
    if (return_val == 1) then
        num_hits = num_hits + 1
    end
    if  (current-old) > 0.5 then
        sim.addStatusbarMessage(string.format('There have been %04d hits in %04d steps, %4.1f steps/second', num_hits, i, steps_per_second))
        old = current
